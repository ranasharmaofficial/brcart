...
class Cat_model extends MY_Model{
	...
	function getChildCatId(){
		$total = 0;
		$fields = "a.id as childcatID";
	...
	function getAllProdByCat($postVal=array())
	// function getAllProdByCat($cat)
	{	
		...
		// $this->db->where(array('a.category_id'=>$cat));
		 ...
		 {
			$this->db->where(array('a.subcategory_id'=>$postVal['subcategory_id'])); 
		 }
		 if($this->uri->segment(5))
		 {
			$this->db->where(array('a.childcategory_id'=>$postVal['childcategory_id'])); 
		 }
