...
class Users_model extends MY_Model{
	...
	public function orderList($limit,$offset)
	  {
		...
		$fields = "a.id as order_id,a.orderid,a.product_id,a.qty,a.seller_id,a.product_price,a.total_price,a.cancel_time,a.delivery_status as deliverystatus,b.*,c.first_name,c.last_name,c.mobile_no,d.name as product_name,d.photo as product_photo";
  ...
  function getAllOrder($limit,$offset)
	...
	function getCanceOrderDetails($params = array()){
		// $fields = "a.*,b.fullname as user_name,b.mobile,b.house_no,b.address,b.landmark,b.city,b.pin,b.state,b.address_type";
		$fields = "a.*";
		$this->db->select($fields);
		$this->db->from(TBL_ORDER_DETAILS.' a');
	...
	function cancelOrder($postVal=array()){
		...
		if($this->db->update(TBL_ORDER_DETAILS,$data)){
			$response = array('status' => STATUS_SUCCESS, 'msg' => 'You have requeseded to cancel your order!');
		}
		return $response;
	}
	...
	public function getDistributorAmount()
    {
        $total = 0;
        $fields = "a.distributor_amount as distributorAmount";
		$this->db->select($fields);
        $this->db->from(TBL_APPLICATION_AMOUNT. ' a');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $row = $query->row_array();
            $total = $row['distributorAmount'];
        }
        return $total;
    }
